...
$order = 0;
if (! empty ($name) && ! empty ($address) && ! empty ($city) && ! empty ($zip) && ! empty ($country) && ! empty ($tracking_number) && ! empty ($referral)) {
    // able to insert into database
    
    $order = $shoppingCart->insertBuyer ( $_POST, $member_id, $item_price);
    if(!empty($order)) {
        if (! empty($cartItem)) {
            if (! empty($cartItem)) {
